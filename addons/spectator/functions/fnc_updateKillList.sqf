...
for "_i" from 1 to 7 do {
    ...
    private _control = (uiNamespace getvariable [QGVAR(labels),[]]) select (_i - 1);
    if(_index < (count GVAR(killedUnits)) && _index >= 0 ) then {
        ...
        if(_deltaTime <= 12 && _i < 7) then {
            if(_killer == _unit || isNull _killer) then {
                _control ctrlSetStructuredText parseText format [
                    "<img image='\a3\Ui_F_Curator\Data\CfgMarkers\kia_ca.paa'/><t font='RobotoCondensed' color='%2'>%1</t>",
                    _dName,
                    _deadSide call CFUNC(sideTohexColor)
                ];
            } else {
                _control ctrlSetStructuredText parseText format [
                    "<t font='RobotoCondensed' color='%4'>%1</t>  <t font='RobotoCondensed'>[%3]</t>  <t font='RobotoCondensed' color='%5'>%2</t>",
                    _kName,
                    _dName,
                    _weapon,
                    _killerSide call CFUNC(sideTohexColor),
                    _deadSide call CFUNC(sideTohexColor)
                ];
            };
        };
